######### Data Wrangling to Create Analytical Dataset
...
# Load required libraries for data manipulation, spatial analysis, and modeling
...
library(foreign)      # Allows reading DBF files
library(readr)        # Fast data reading
library(igraph)      # Network/graph analysis for clustering

# ==============================================================================
# SECTION 2: OCEAN ACIDIFICATION DATA PREPARATION
# ==============================================================================

# -----------------------------------------------------------------------------
# 2.1 Load and Filter OA Dataset
# -----------------------------------------------------------------------------
...
# Calculate time ranges for each dataset
full <- full %>% group_by(dataset_id) %>% mutate(min_time = min(time_utc, na.rm = TRUE),
                                                   max_time = max(time_utc, na.rm = TRUE))

# -----------------------------------------------------------------------------
# 2.2 Identify Unique Coordinates and Filter by Time Series Length
# -----------------------------------------------------------------------------
...
# Convert to spatial object and write to file for mapping
...
# -----------------------------------------------------------------------------
# 2.3 Clean and Filter OA Data
# -----------------------------------------------------------------------------
# Extract only points where alkalinity is nonzero.
# Fix trailing zeros in coordinates (e.g., "45.1200" -> "45.12")
...
# Filter to only rows with alkalinity data (reduces dataset from ~14 million to 12152 sites)
full <- full %>% filter(!is.na(ta_umolkg))
...
# -----------------------------------------------------------------------------
# 2.4 Aggregate to Daily Values
# -----------------------------------------------------------------------------
# Several observations appear to have repeated observations per day over several hours. 
# Find mean per day to avoid pseudoreplication.
...
# Convert to shapefile; keep longitude/latitude as attributes
full_daily <- full_daily %>% st_as_sf(coords = c("longitude","latitude"), crs = 4326, remove = FALSE)

# -----------------------------------------------------------------------------
# 2.5 Export Daily Data (Optional - commented out)
# -----------------------------------------------------------------------------
# # Filter by region for regional analyses
...
# Export shallowest point per day (minimum depth) for quick visualization
full_daily_shallow <- full_daily %>% group_by(date, longitude, latitude) %>% filter(depth_m == min(depth_m))
st_write(full_daily_shallow, "/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/OA/daily_alk_minDepth.csv")


# ==============================================================================
# SECTION 3: INLAND ALKALINITY DATA PREPARATION
# ==============================================================================
# This section processes inland (terrestrial) alkalinity data from multiple sources
# to create watershed-level alkalinity estimates that can be joined to ocean data.

# -----------------------------------------------------------------------------
# 3.1 GLORICH Dataset (Global River Chemistry)
# -----------------------------------------------------------------------------
# GLORICH is a global river chemistry database with alkalinity measurements
...
# -----------------------------------------------------------------------------
# 3.2 SWAMP Dataset (California State)
# -----------------------------------------------------------------------------
# SWAMP (Surface Water Ambient Monitoring Program) is California's state water quality monitoring program
...
swamp$ta_umolkg <- swamp$Result * 10 * 2  # Convert mg/L to umol/kg (multiply by 10 for conversion, times 2 for carbonate equivalent)
...
# -----------------------------------------------------------------------------
# 3.3 USGS Dataset (US Geological Survey)
# -----------------------------------------------------------------------------
# USGS water quality data for California, Washington, and Oregon
...
new_usgs <- new_usgs %>% mutate(alkalinity_umolkg = 2*10*alkalinity_mgL)  # Convert mg/L to umol/kg

# Ensure that all rows have an alkalinity value (= ensure that USGS data retrieval was correct); all rows in this dataset do have alk.
new_usgs <- new_usgs %>% filter(!is.na(alkalinity_mgL))

# Remove one anomalously high value
new_usgs <- new_usgs[-16380,]

# Load USGS gauge site information to determine the lat/lon of USGS alkalinity
...
# -----------------------------------------------------------------------------
# 3.4 Combine Inland Alkalinity Sources
# -----------------------------------------------------------------------------
# Finish reformatting USGS and SWAMP datasets so that they can be rbinded
...
# -----------------------------------------------------------------------------
# 3.5 Aggregate Inland Data by Watershed (HUC Levels)
# -----------------------------------------------------------------------------
# Aggregate alkalinity data by watershed (HUC8, HUC10, HUC12) for joining to ocean data
...
# -----------------------------------------------------------------------------
# 3.6 Join Inland Alkalinity to Watersheds
# -----------------------------------------------------------------------------
# Convert USGS + SWAMP alkalinity points to sf object
ta_land_shp <- st_as_sf(ta_land, coords = c("Lon","Lat"), crs = 4326)

# Join points to watershed using spatial join (points within polygons)
sf_use_s2(FALSE)
...
# Some rows are repeated for some reason, perhaps because alk points were on the border of two hucs, so ensure that only one row per unique values of all columns
ta_in_huc8 <- ta_in_huc8 %>% distinct()
ta_in_huc10 <- ta_in_huc10 %>% distinct()
ta_in_huc12 <- ta_in_huc12 %>% distinct()

# -----------------------------------------------------------------------------
# 3.7 Calculate Watershed-Level Statistics
# -----------------------------------------------------------------------------
# Mean, maximum, and minimum alkalinity per HUC
# Note: DO NOT use na.rm = TRUE because there are alkalinity values at all points 
# and we don't want to conflate 0 alkalinity with 0 points within HUC
huc8_ta <- ta_in_huc8 %>% as.data.frame() %>% group_by(HUC8) %>% summarize(mean_ta_huc8 = mean(ta_umolkg),
                                                                           max_ta_huc8 = max(ta_umolkg),
                                                                           min_ta_huc8 = min(ta_umolkg)) %>% right_join(huc8, by = "HUC8", multiple = "all") %>% st_as_sf() # Do not remove NA values as we will be imputing missing HUC8 TA values
...
st_write(huc8_ta, "/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/WBD/All/HUC8/huc8_Wcoast_w_alk2.shp")
...
# -----------------------------------------------------------------------------
# 3.8 Join Watershed Alkalinity to Ocean Data
# -----------------------------------------------------------------------------
# Calculate, for OA dataset, watershed-based alkalinity at HUC8, HUC10, and HUC12 levels
# Filter OA daily dataset to those points within 10 km of coast
full_daily_coast <- full_daily %>% dplyr::filter(distance_offshore <=10)

# Find nearest HUC8 to each OA point
sf_use_s2(FALSE)
...
OA_join <- cbind(full_daily_coast, (huc8_ta[nearhuc8,] %>% as.data.frame() %>% dplyr::select(-geometry)))
...
st_write(OA_join, "/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/OA/OA_coastal_alk.shp")
OA_join <- st_read('/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/OA/OA_coastal_alk.shp')


# ==============================================================================
# SECTION 4: RIVER FLOW DATA PROCESSING
# ==============================================================================
# This section processes river flow data from NHDPlus (National Hydrography Dataset)
# to create flow estimates that can be joined to ocean data.

# -----------------------------------------------------------------------------
# 4.1 Load NHD River Network Data
# -----------------------------------------------------------------------------
# Load NHD per region (PNW = Pacific Northwest, CA = California)
PNW <- st_read("/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/Rivers/PNW/NHDPlus/Hydrography/NHDFlowline.shp")
CA <- st_read("/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/Rivers/CA/NHDPlus/NHDSnapshot/Hydrography/NHDFlowline.shp")
...
# For PNW
fp_ls_pnw <- list.files("/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/Rivers/PNW/EROMExtension/EROM_monthly/", full.names = TRUE)
flow_ls_pnw <- lapply(fp_ls_pnw, flow_pick)

# Plot showing imputed vs observed points (imputed in red)
...
# For CA
fp_ls_ca <- list.files("/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/Rivers/CA/EROMExtension/EROM_Monthly/", full.names = TRUE)
...
ma_pnw <- read.dbf("/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/Rivers/PNW/EROMExtension/EROM_MA0001.DBF")
flow_pnw <- ma_pnw %>% dplyr::select(c(ComID, Q0001E)) %>% full_join(flow_permonth_pnw, by = "ComID")
colnames(flow_pnw)[2] <- "ma"

ma_ca <- read.dbf("/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/Rivers/CA/EROMExtension/EROM_MA0001.DBF")
...
st_write(PNW, "/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/Rivers/PNW/NHDPlus/Hydrography/NHDFlowline_plus_flow.shp")
st_write(PNW_rivers, "/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/Rivers/PNW/NHDPlus/Hydrography/NHDRivers_plus_flow.shp")
st_write(PNW_coasts, "/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/Rivers/PNW/NHDPlus/Hydrography/NHDCoasts.shp")
st_write(CA, "/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/Rivers/CA/NHDPlus/NHDSnapshot/Hydrography/NHDFlowline_plus_flow.shp")
st_write(CA_rivers, "/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/Rivers/CA/NHDPlus/NHDSnapshot/Hydrography/NHDRivers_plus_flow.shp")
st_write(CA_coasts, "/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/Rivers/CA/NHDPlus/NHDSnapshot/Hydrography/NHDCoasts.shp")

PNW <- st_read("/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/Rivers/PNW/NHDPlus/Hydrography/NHDFlowline_plus_flow.shp")
PNW_rivers <- st_read("/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/Rivers/PNW/NHDPlus/Hydrography/NHDRivers_plus_flow.shp")
PNW_coasts <- st_read("/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/Rivers/PNW/NHDPlus/Hydrography/NHDCoasts.shp")
CA <- st_read("/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/Rivers/CA/NHDPlus/NHDSnapshot/Hydrography/NHDFlowline_plus_flow.shp")
CA_rivers <- st_read("/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/Rivers/CA/NHDPlus/NHDSnapshot/Hydrography/NHDRivers_plus_flow.shp")
CA_coasts <- st_read("/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/Rivers/CA/NHDPlus/NHDSnapshot/Hydrography/NHDCoasts.shp")
...
st_write(coastal_riv, "/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/NHDcoastal_rivers.shp")
st_write(coasts, "/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/NHDCoastline.shp")
st_write(rivers, "/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/NHDRivers.shp")

coastal_riv <- st_read("/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/NHDcoastal_rivers.shp")
coasts <- st_read("/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/NHDCoastline.shp")
rivers <- st_read("/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Hydrology/NHDRivers.shp")

# Join flow data to main frame
OA_join <- st_transform(OA_join, crs = st_crs(coastal_riv))
D <- st_distance(OA_join, coastal_riv)

# --- Modified: Find ALL coastal rivers within 10 km of each point ---
# Convert 15 km to meters for distance comparison
threshold_m <- 15000

# For each OA point, find all rivers within 10 km
# Create a list to store nearby rivers for each point
nearby_rivers_list <- list()
num_rivers_within_15km <- numeric(nrow(OA_join))
composite_fw_expose <- numeric(nrow(OA_join))
composite_fw_expose_month <- numeric(nrow(OA_join))

# Get flow data (mean annual flow in m3/s) for each river
# Include monthly flow columns (columns 16-27 are the 12 months)
coastal_riv_df <- coastal_riv %>% as.data.frame() %>% dplyr::select(c(ComID, FDATE, REACHCODE, FCODE, ma, 16:27))
coastal_riv_df$ma_ms <- coastal_riv_df$ma * 0.0283168  # Convert cfs to m3/s

# Get the actual column names for monthly flow columns (columns 6-17 in the data.frame)
month_col_names_orig <- names(coastal_riv_df)[6:17]

# Convert monthly flows to m3/s - handle both numeric and named columns
for (j in 1:12) {
  col_name <- month_col_names_orig[j]
  if (!is.na(col_name) && col_name %in% names(coastal_riv_df)) {
    new_col_name <- paste0("m", sprintf("%02d", j), "_ms")
    coastal_riv_df[[new_col_name]] <- coastal_riv_df[[col_name]] * 0.0283168
  }
}

for (i in 1:nrow(OA_join)) {
  # Get distances from this point to all rivers
  dist_vec <- as.numeric(D[i, ])
  
  # Find rivers within 15 km (threshold_m meters)
  within_threshold <- dist_vec <= threshold_m
  
  # Count how many rivers are within 15 km
  num_rivers_within_15km[i] <- sum(within_threshold)
  
  if (sum(within_threshold) > 0) {
    # Get indices of rivers within 15 km
    riv_indices <- which(within_threshold)
    riv_distances <- dist_vec[riv_indices]
    riv_flows <- coastal_riv_df$ma_ms[riv_indices]
    
    # Calculate fw_expose for each nearby river (flow/distance in km)
    # Using the same approach as EDA.R: fw_expose = Q/D
    # Distance in km = distance in m / 1000
    riv_dist_km <- riv_distances / 1000
    
    # Calculate exposure for each river and sum them
    # Handle points on or very close to rivers: reclassify distance = 0 as 1e-9 km
    riv_dist_km_adj <- ifelse(riv_dist_km <= 0, 0.000000001, riv_dist_km)
    
    # Calculate exposure for each river and sum them
    river_exposures <- riv_flows / riv_dist_km_adj
    
    # Composite index: sum of all (flow/distance) for rivers within 15 km
    composite_fw_expose[i] <- sum(river_exposures, na.rm = TRUE)
    
    # --- Monthly flow calculation ---
    # Get the month from OA_join date
    point_month <- month(OA_join$date[i])
    
    # Get the monthly flow column name for this month (e.g., "m01_ms" for January)
    month_col_name <- paste0("m", sprintf("%02d", point_month), "_ms")
    
    # Get the monthly flow values for rivers within threshold
    riv_flows_month <- coastal_riv_df[[month_col_name]][riv_indices]
    
    # Calculate monthly exposure for each river and sum them
    river_exposures_month <- riv_flows_month / riv_dist_km_adj
    
    # Composite monthly index: sum of all (flow/distance) for rivers within 15 km
    composite_fw_expose_month[i] <- sum(river_exposures_month, na.rm = TRUE)
  } else {
    composite_fw_expose[i] <- NA
    composite_fw_expose_month[i] <- NA
  }
  
  # Progress indicator
  if (i %% 1000 == 0) {
    cat("Processed", i, "of", nrow(OA_join), "points\n")
  }
}


# Add new columns to OA_join
OA_join$num_rivers_15km <- num_rivers_within_15km
OA_join$composite_fw_expose <- composite_fw_expose
OA_join$composite_fw_expose_month <- composite_fw_expose_month

# Also keep the original nearest river approach for comparison
nearest <- max.col(-D)

# Directly assign flow values from the nearest river
OA_join$flowYear_cfs <- coastal_riv_df$ma[nearest]
OA_join$flowYear_m3s <- coastal_riv_df$ma_ms[nearest]

# Get monthly flow for each point based on its month
# Extract month from date
OA_join$month <- month(OA_join$date)

# Assign flowMonth_cfs based on the month of each point
for (i in 1:nrow(OA_join)) {
  pt_month <- OA_join$month[i]
  month_col <- paste0("0", pt_month)  # e.g., "01" for January
  if (month_col %in% names(coastal_riv_df)) {
    OA_join$flowMonth_cfs[i] <- coastal_riv_df[[month_col]][nearest[i]]
  }
}
...
fp_ls_soilPh <- list.files("/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/SoilpH/", pattern = ".csv", full.names = TRUE)
...
OA_join2 <- left_join(OA_join, soilPh_HUC8_mean, by = "HUC8")
OA_join2 <- left_join(OA_join2, soilPh_HUC8_max, by = "HUC8")
...
upwell <- read.csv("/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Upwell/CUTI_daily.csv")
upwellN <- read.csv("/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/Upwell/BEUTI_daily.csv")
# Extract coordinates from point geometry in OA_join master dataset
OA_join2$lat <- st_coordinates(OA_join2)[,2]
OA_join2$lon <- st_coordinates(OA_join2)[,1]
...
# 
# # Impute mean_ta_huc8 for HUC8 in PNW with missing TA values from the soil pH of those regions based on linear relationship between HUC8 TA and soil pH in other watersheds where data are available
# ggplot(OA_join2, aes(x = b0.HUC8mean, y = men_t_hc8)) + geom_point() + geom_smooth(method = "lm")

# Count missing
sum(is.na(OA_join2$mean_ta_huc8))

# Remove HUC10-HUC12 variables
OA_join2 <- OA_join2[,-c(21:28)]

# Reset colnames
colnames(OA_join2) <- c("date","depth_m","dataset_id","seaTemp","seaSal","seaPH","seaTCO2","seaTA","seaDO","seaChl","seaSi","seaNH4","seaNO3","seaNO2","seaPO3","distanceSho",
                  "huc8","ta_hucMean","ta_hucMax","ta_hucMin","numberRiv_15km","composite_fw_expose","composite_fw_exp_mo","flowYear_cfs", "flowYear_m3s", "month","distanceRiv",
                  "soilpH_hucMean","soilpH_hucMax","lat","lon", "latInteger","year","day","upwell","upwellN","geometry")


# # Prepare data for modeling (rows with both values)
# fit_df <- OA_join2 %>%
#   filter(!is.na(ta_hucMean) & !is.na(soilpH_hucMean))
# 
# # Fit linear model (simple and interpretable)
# lm_ta <- lm(ta_hucMean ~ soilpH_hucMean, data = fit_df)
# summary(lm_ta)  # optional: inspect fit
# 
# # Identify rows to impute (missing TA but have b0.HUC8mean)
# miss_idx <- which(is.na(OA_join2$mean_ta_huc8) & !is.na(OA_join2$b0.HUC8mean))
# 
# # Predict and constrain predictions to observed TA range
# pred_vals <- predict(lm_ta, newdata = OA_join2[miss_idx, , drop = FALSE])
# pred_vals <- pmax(pred_vals, min(fit_df$mean_ta_huc8, na.rm = TRUE))
# pred_vals <- pmin(pred_vals, max(fit_df$mean_ta_huc8, na.rm = TRUE))
# 
# # Apply imputations and flag them
# OA_join2$mean_ta_huc8_imputed <- FALSE
# OA_join2$mean_ta_huc8_imputed[miss_idx] <- TRUE
# OA_join2$mean_ta_huc8[miss_idx] <- pred_vals
# 
# # Highlight imputed points in red
# if(!"mean_ta_huc8_imputed" %in% names(OA_join2)) OA_join2$mean_ta_huc8_imputed <- FALSE
# OA_join2$mean_ta_huc8_imputed <- as.logical(OA_join2$mean_ta_huc8_imputed)
# message('Number imputed (flag TRUE): ', sum(OA_join2$mean_ta_huc8_imputed, na.rm = TRUE))
# 
# ggplot(OA_join2, aes(x = b0.HUC8mean, y = men_t_hc8)) +
#   geom_point(data = dplyr::filter(OA_join2, !mean_ta_huc8_imputed), aes(), color = 'black', alpha = 0.6, size = 1.5) +
#   geom_smooth(data = dplyr::filter(OA_join2, !mean_ta_huc8_imputed), method = 'lm', se = TRUE, color = 'grey') +
#   geom_point(data = dplyr::filter(OA_join2, mean_ta_huc8_imputed), aes(), color = 'red', alpha = 1, size = 2) +
#   labs(title = 'mean_ta_huc8 vs b0.HUC8mean — imputed points highlighted', x = 'b0.HUC8mean', y = 'mean_ta_huc8') +
#   theme_minimal()

# Verify
sum(is.na(OA_join2$mean_ta_huc8))
table(OA_join2$mean_ta_huc8_imputed)


# --- Cluster points within 50m on same date and depth bin, then average them ---
# Create a unique date column (date only, no time)
OA_join2$date_only <- as.Date(OA_join2$date)

# Create depth bins (every 15 meters)
# Bin 0: depths 0-15m (mixed layer, inclusive of 15)
# Bin 1: depths 16-30m, Bin 2: 31-45m, etc.
OA_join2$depth_bin <- ifelse(OA_join2$depth_m <= 15, 0, floor((OA_join2$depth_m - 15) / 15) + 1)

# Create spatial clusters: for each unique date and depth bin, find points within 50m of each other
# First, convert to projected coordinates (meters) for distance calculation
OA_join2_proj <- st_transform(OA_join2, crs = 32610)  # UTM Zone 10N

# Create a cluster ID for points within 50m on the same date and depth bin
OA_join2$cluster_id <- NA
cluster_counter <- 1

# Loop through each unique combination of date and depth bin
for (d in unique(OA_join2$date_only)) {
  for (db in unique(OA_join2$depth_bin[OA_join2$date_only == d])) {
    # Get points for this date and depth bin
    date_depth_idx <- which(OA_join2$date_only == d & OA_join2$depth_bin == db)
    
    if (length(date_depth_idx) > 1) {
      # Calculate pairwise distances between points on this date/depth
      dist_mat <- st_distance(OA_join2_proj$geometry[date_depth_idx])
      
      # Convert to numeric matrix (meters) - sf returns units objects
      dist_mat <- as.numeric(dist_mat)
      attr(dist_mat, "units") <- NULL
      dim(dist_mat) <- c(length(date_depth_idx), length(date_depth_idx))
      
      # Find clusters using connected components approach
      # Points within 100m of each other are connected
      within_100m <- dist_mat <= 100
      
      # Use graph-based clustering
      g <- graph_from_adjacency_matrix(within_100m, mode = "undirected")
      clusters <- components(g)$membership
      
      # Assign cluster IDs - group by cluster membership
      unique_clusters <- unique(clusters)
      for (c in unique_clusters) {
        cluster_points <- which(clusters == c)
        cluster_id <- paste0("C", cluster_counter)
        OA_join2$cluster_id[date_depth_idx[cluster_points]] <- cluster_id
        cluster_counter <- cluster_counter + 1
      }
    } else if (length(date_depth_idx) == 1) {
      OA_join2$cluster_id[date_depth_idx] <- paste0("C", cluster_counter)
      cluster_counter <- cluster_counter + 1
    }
  }
}

# Group by cluster_id and calculate mean values for all columns
OA_join2_no_geom <- st_drop_geometry(OA_join2)

# Identify numeric columns to average (exclude metadata columns)
cols_to_average <- names(OA_join2_no_geom)[sapply(OA_join2_no_geom, is.numeric)]
cols_to_average <- cols_to_average[!cols_to_average %in% c("dataset_id")]

# Calculate grouped means - drop geometry for this step
OA_join2_clustered <- OA_join2_no_geom %>%
  group_by(cluster_id) %>%
  summarize(across(all_of(cols_to_average), ~mean(., na.rm = TRUE)),
            date_only = first(date_only),
            depth_bin = first(depth_bin),
            dataset_id = first(dataset_id),
            .groups = "drop")

# For spatial columns, take the centroid of each cluster (geometric centroid)
cluster_centroids <- OA_join2 %>%
  group_by(cluster_id) %>%
  summarize(geometry = st_centroid(st_combine(geometry)))

# Join the averaged values with the centroid geometry
OA_join2 <- left_join(OA_join2_clustered, cluster_centroids, by = "cluster_id")

cat("Clustered", nrow(OA_join2_clustered), "points into", length(unique(OA_join2$cluster_id)), "clusters\n")


# Write dataset to file
st_write(OA_join2, "/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/OA/OA_master_clustered_26April2026.shp")
write.csv(OA_join2 %>% as.data.frame() %>% dplyr::select(-geometry), "/Users/rachelcarlson/Documents/Berkeley/Research/Postdoc-2022-present/OA_dataset/Data/OA/OA_master_clustered_26April2026.csv")
